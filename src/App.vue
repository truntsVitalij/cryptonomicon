...
<script>
	export default {
		...
		created() {
			const localData = localStorage.getItem("cryptonomicon-list");
		...
		methods: {
			addTicker() {
				...
				setInterval(async () => {
					const f = await fetch(
						`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=e657430f8e706d2fe83523f2b793ba3cfce88c856f8e1c446d38a255bbb55f32`
					);
					const result = await f.json();
					this.tickersList.find((t) => t.name == newTicker.name).price =
			...
			handleDelete(ticker) {
				this.tickersList = this.tickersList.filter((t) => t !== ticker);
			},
			clearSelectedTicker() {
